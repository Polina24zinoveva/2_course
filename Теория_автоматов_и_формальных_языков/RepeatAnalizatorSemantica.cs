...
//repeat a[5] := 3.14 - t until d;



namespace LR__automats_
{
    internal class RepeatAnalizatorSemantica
    {
        private enum States
        {
            S,
        ...
        public static (List<List<string>>, List<List<string>>, string result) Main(string s)
        {
            States states = States.S;
            s = s.ToLower();
            string identificator = "";
            string constanta = "";
            int startIdentificatorPosition = 0;
            int startConstantaPosition = 0;
            bool divOrModStates = false;
            List<List<string>> ListIdentificators = new List<List<string>>();
            List<List<string>> ListConst = new List<List<string>>();
            string[] kindOfIdent = { "массив", "индекс", "константа выражения" };
            string[] kindOfConst = { "индекс", "константа выражения" };
            string[] typeOfConst = { "целая", "вещественная" };
            string[] reservedName = { "repeat", "until", "and", "or", "xor", "div", "mod" };
                    ...
                    case States.B1:
                        {
                            ...
                            else if (s[i] == '[')
                            {
                                ...
                                {
                                    throw new ExceptionWithPosition("Длина идентификатора больше 8 символов!", startIdentificatorPosition);
                                }
                                if ((identificator == reservedName[0]) || (identificator == reservedName[1]) || (identificator == reservedName[2]) || (identificator == reservedName[3]) || (identificator == reservedName[4]) || (identificator == reservedName[5]) || (identificator == reservedName[6]))
                                {
                                    throw new ExceptionWithPosition("Введено зарезервированное слово!", startIdentificatorPosition);
                                }
                                ListIdentificators.Add(new List<string> { identificator, kindOfIdent[0] });
                                identificator = "";
                                startIdentificatorPosition = 0;
                                i++;
                            }
                            else if (s[i] == ' ') { i++; }
                            else if (s[i] == ':')
                            {
                                ...
                                ListIdentificators.Add(new List<string> { identificator, kindOfIdent[2] });
                                identificator = "";
                                startIdentificatorPosition = 0;
                            }
                    ...
                    case States.B3:
                        {
                            ...
                            else if (s[i] == ']')
                            {
                                ...
                                for (int a = 0; a < ListIdentificators.Count; a++)
                                {
                                    if ((identificator == ListIdentificators[a][0]) && (ListIdentificators[a][1] != kindOfIdent[1]))
                                    {
                                        throw new ExceptionWithPosition("Ошибка: данный идентификатор другого вида", startIdentificatorPosition);
                                    }
                                }
                                ListIdentificators.Add(new List<string> { identificator, kindOfIdent[1] });
                                identificator = "";
                                startIdentificatorPosition = 0;
                                i++;
                            }
                    ...
                    case States.B5:
                        {
                            ...
                            {
                                states = States.B8;
                                if ((Convert.ToInt32(constanta) < -32768) && (Convert.ToInt32(constanta) > 32767))
                                {
                                    throw new ExceptionWithPosition("Число не в диапазоне от -32768 до +32767", startIdentificatorPosition);
                                }
                                ListConst.Add(new List<string> { constanta, kindOfConst[0], typeOfConst[0] });
                                constanta = "";
                                startConstantaPosition = 0;
                                i++;
                            }
                            else if (s[i] == '+' || s[i] == '-')
                            {
                    ...
                    case States.D1:
                        {
                            if (s[i] == ' ')
                            {
                                ...
                                for (int a = 0; a < ListIdentificators.Count; a++)
                                {
                                    if ((identificator == ListIdentificators[a][0]) && (ListIdentificators[a][1] != kindOfIdent[2]))
                                    {
                                        throw new ExceptionWithPosition("Ошибка: данный идентификатор другого вида", startIdentificatorPosition);
                                    }
                                }
                            ...
                            else if (s[i] == '[')
                            {
                                ...
                                for (int a = 0; a < ListIdentificators.Count; a++)
                                {
                                    if ((identificator == ListIdentificators[a][0]) && (ListIdentificators[a][1] != kindOfIdent[0]))
                                    {
                                        throw new ExceptionWithPosition("Ошибка: данный идентификатор другого вида", startIdentificatorPosition);
                                    }
                                }
                    ...
                    case States.E2:
                        {
                            ...
                            {
                                states = States.J;
                                if ((double.Parse(constanta, CultureInfo.InvariantCulture) < -32768) || (double.Parse(constanta, CultureInfo.InvariantCulture) > 32767))
                                {
                                    throw new ExceptionWithPosition("Число не в диапазоне от -32768 до +32767", startConstantaPosition);
                                }
                                ListConst.Add(new List<string> { constanta, kindOfConst[1], typeOfConst[1] });
                                constanta = "";
                                startConstantaPosition = 0;
                            }
                            else if ((s[i] == 'd') || (s[i] == 'm'))
                            {
                                states = States.G; //выход из операнда
                                if (constanta.Contains('.'))
                                {
                                    throw new ExceptionWithPosition("При наличии mod или div допускаются только целые константы", startConstantaPosition);
                                }
                                if ((double.Parse(constanta, CultureInfo.InvariantCulture) < -32768) || (double.Parse(constanta, CultureInfo.InvariantCulture) > 32767))
                                {
                                    throw new ExceptionWithPosition("Число не в диапазоне от -32768 до +32767", startConstantaPosition);
        ...
        //УСЛОВИЕ
            ...
            string resultString = "Строка правильная!";
            for (int a = 0; a < ListIdentificators.Count; a++)
            {
                for (int b = a + 1; b < ListIdentificators.Count; b++)
                {
                    if (ListIdentificators[a][0] == ListIdentificators[b][0])
                    {
                        ListIdentificators.Remove(ListIdentificators[b]);
                    }
                }
            }
            var result = (ListIdentificators, ListConst, resultString);
            return result;
        }
    }
}
